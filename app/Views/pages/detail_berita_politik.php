                        ...
                        <div class="col-lg-8">
                            <div>
                                ...
                                <p class="fs-13 text-muted mb-0 mt-2">
                                    <span class="mr-1"><i class="fa-solid fa-clock"></i> </span><?= tgl_indo_model_2(date($detailPolitik['tanggal_buat'])); ?>
                                    <span class="mr-1 ml-1"><i class="fas fa-eye"></i> </span><?= $detailPolitik['banyak_dilihat']; ?> Views
                                </p>
                            ...
                            <div class="post-comment-section">
                                ...
                                <div class="comment-section">
                                    <?php if (in_groups('User') || in_groups('Admin')) : ?>
                                        ...
                                        <div class="testimonial">
                                            <div>
                                                ...
                                                <button class="btn btn-primary" id="komentarPolitik">Kirim</button>
                                            </div>
                                        </div>
                                    <?php else : ?>
                                        <h5 class="font-weight-600">Kolom Komentar</h5>
                                    <?php endif; ?>
                                    <div id="comments">
                                        <?php foreach ($komentarPolitik as $i => $value) { ?>
...
<script type="text/javascript">
    $('#komentarPolitik').on('click', function() {
        var isi_komentar = document.getElementById('isi_komentar');
        $.ajax({
            url: '/home/komentar_politik1',
            method: 'POST',
            data: {
                id_berita: <?= $detailPolitik['id_berita'] ?>,
    ...
    function getKomentarPolitik() {
        $.ajax({
            url: '/home/get_komentar_politik/<?= $detailPolitik['id_berita'] ?>',
            method: 'GET',
    ...
    $('.content-wrapper').on('click', '.balas-komentar', function() {
        ...
        if (!document.querySelectorAll('.textarea-balas')[index].querySelector('textarea')) {
            ...
            <button class="btn btn-primary mt-3 balaskomentarPolitik" id="balaskomentarPolitik" data-id="${komentar_id}" data-index="${index}">Kirim</button>`
        } else {
            document.querySelectorAll('.textarea-balas')[index].innerHTML = ''
        }
    });

    $('body').on('click', '.balaskomentarPolitik', function() {
        var index = $(this).data('index');
        var komentar_id = $(this).data('id');
